...
class ImageDescriptionService(app: Application) {
    ...
    private suspend fun ensureFeatureReady() {
        val status = try { imageDescriber.checkFeatureStatus().await() } catch (e: Exception) { FeatureStatus.UNAVAILABLE }
        when (status) {
            ...
            FeatureStatus.DOWNLOADABLE, FeatureStatus.DOWNLOADING -> suspendCancellableCoroutine { cont ->
                imageDescriber.downloadFeature(object : DownloadCallback {
                    ...
                    override fun onDownloadCompleted() { if (cont.isActive) cont.resume(Unit, onCancellation = null) }
                    override fun onDownloadFailed(exception: GenAiException) { if (cont.isActive) cont.cancel(exception) }
                })
            }
    ...
    fun close() { imageDescriber.close() }
} 